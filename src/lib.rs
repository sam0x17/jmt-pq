...
//! This module implements [`JellyfishMerkleTree`] backed by storage module. The tree itself
//! doesn't persist anything, but realizes the logic of R/W only. The write path will produce
//! all the intermediate results in a batch for storage layer to commit and the read path will
//! return results directly. The public APIs are only [`new`], [`put_value_sets`],
//! [`put_value_set`] and [`get_with_proof`]. After each put with a `value_set` based on a
//! known version, the tree will return a new root hash with a [`TreeUpdateBatch`] containing
//! all the new nodes and indices of stale nodes.
//!
//! A Jellyfish Merkle Tree itself logically is a 512-bit sparse Merkle tree with an
//! optimization that any subtree containing 0 or 1 leaf node will be replaced by that leaf
//! node or a placeholder node with default hash value. With this optimization we can save CPU
//! by avoiding hashing on many sparse levels in the tree. Physically, the tree is structurally
//! similar to the modified Patricia Merkle tree of Ethereum but with some modifications. A
...
//! ```
//!
//! A Jellyfish Merkle Tree consists of [`InternalNode`] and [`LeafNode`]. [`InternalNode`] is
//! like branch node in ethereum patricia merkle with 16 children to represent a 4-level binary
//! tree and [`LeafNode`] is similar to that in patricia merkle too. In the above figure, each
//! `bell` in the jellyfish is an [`InternalNode`] while each tentacle is a [`LeafNode`]. It is
//! noted that Jellyfish merkle doesn't have a counterpart for `extension` node of ethereum
//! patricia merkle.
//!
...
/// Contains types used to bridge a [`JellyfishMerkleTree`](crate::JellyfishMerkleTree) to the
/// backing storage recording the tree's internal data.
pub mod storage {
...
/// An error that occurs when the state root for a requested version is missing (e.g., because
/// it was pruned).
#[derive(Debug)]
...
#[cfg_attr(any(test), derive(Arbitrary))]
// This needs to be public for the fuzzing/Arbitrary feature, but we don't really want it to
// be, so #[doc(hidden)] is the next best thing.
#[doc(hidden)]
...
impl KeyHash {
    ...
    ///
    /// # 🚨 Danger 🚨
    /// If you will use ics23 non-existence proofs, you must ensure that the key is non-empty
    /// before calling this function.
    pub fn with<H: SimpleHasher>(key: impl AsRef<[u8]>) -> Self {
        let key_hash = Self(H::hash(key.as_ref()));
        // Adding a tracing event here allows cross-referencing the key hash with the original
        // key bytes when looking through logs.
...
/// `Digest` for broader compatibility.
